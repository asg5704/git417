...
function generateResults({ records }) {
  ...
  records.forEach(({begindate = '-', enddate = '-', title, url, poster}) => {
    ...
    // jQuery version
    var img = $('<img />')
      .addClass('h-16 w-16 rounded-full mx-auto md:mx-0 md:mr-6')
      .attr('src', !poster ? './assets/images/empty.png' : poster.imageurl)
      .attr('alt', !poster ? 'No poster' : poster.caption)
    ...
    var article = $('<article></article>')
      .addClass('md:flex bg-gray-900 rounded-lg p-6 mr-4 mt-4 shadow max-w-3xl')
      .html(`<div class="flex-col"><h3 class="text-gray-400 text-left"><a href="${url}">${title}</a></h3><span class="block text-xs text-white">${begindate} thru ${enddate}</span></div>`)
      .prepend(img)
